...
#
# Supports:
# - Parrot Security OS / Debian based guests
# - VirtualBox Guest Additions package check (apt)
# - Kernel header check
# - DKMS check (if available)
# - Missing /dev/vboxguest and /dev/vboxuser fix
# - vboxguest module loading
# - virtualbox-guest-utils service restart
# - VBoxClient clipboard restart
# - VBoxClient drag-and-drop restart
# - X11 clipboard test using xclip
# - Permanent reboot fix (systemd + udev)
# - User login autostart fix
# - Clear PASS / WARN / FAIL report
#
# Modes:
#   bash fix-parrot-vbox-clipboard.sh                 (safe mode)
#   bash fix-parrot-vbox-clipboard.sh --aggressive    (purge + reinstall)
#   bash fix-parrot-vbox-clipboard.sh --diagnose-only (no changes)
#
# Important:
# This script fixes Parrot/Linux guest-side issues.
# VirtualBox host-side menu must still be set manually:
...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      echo "Usage: bash fix-parrot-vbox-clipboard.sh [--aggressive] [--diagnose-only]"
      echo
      ...
      echo "--diagnose-only checks and prints report without changing the system."
      exit 0
      ;;
  esac
done
...
require_not_root() {
  if [ "$EUID" -eq 0 ]; then
    ...
    echo
    echo "Wrong:"
    echo "sudo bash fix-parrot-vbox-clipboard.sh"
    exit 1
  fi
}
...
# Parrot may run a custom kernel. Try the exact matching headers first,
# then fall back to the generic Debian meta-package.
install_kernel_headers() {
  ...
  if [ "$MODE" = "diagnose" ]; then
    if apt-cache show "$exact" >/dev/null 2>&1; then
      warn "$exact available but not installed"
    else
      warn "$exact not in repos. Generic linux-headers-amd64 will be tried in fix mode."
    fi
    return 1
  fi
  ...
  if apt-cache show "$exact" >/dev/null 2>&1; then
    ...
    warn "$exact not found in repositories. Falling back to linux-headers-amd64."
...
start_vbox_clients() {
  if [ -z "${DISPLAY:-}" ]; then
    fail "DISPLAY is not set. Start the Parrot desktop GUI and run the script again."
    return 1
  fi
...
[Unit]
Description=EWB Fix VirtualBox guest device nodes (Parrot)
...
aggressive_reinstall() {
  ...
  info "Reinstalling required headers and tools..."
  install_kernel_headers
  run_cmd sudo apt install -y virtualbox-guest-x11 virtualbox-guest-utils xclip
...
info "Log file: $LOG"
echo
echo "Before testing, the VirtualBox top menu MUST be set to:"
echo "  Devices -> Shared Clipboard -> Bidirectional"
echo "  Devices -> Drag and Drop    -> Bidirectional"
echo "(This is a host-side setting. The script cannot change it.)"
echo
...
if [ -f /etc/os-release ]; then
  ...
  if echo "${PRETTY_NAME:-} ${ID:-} ${NAME:-}" | grep -qi "parrot"; then
    pass "Parrot Security OS detected"
  else
    warn "This does not look like Parrot. Script still targets Debian/apt based guests."
  fi
fi
...
if [ "${XDG_SESSION_TYPE:-}" = "x11" ]; then
  ...
  warn "Session is not X11. If clipboard fails, log out and choose an X11/Xorg session."
fi
...
if ! command -v apt >/dev/null 2>&1; then
  fail "APT not found. This script supports Parrot/Debian based systems."
  exit 1
fi
...
if command -v dkms >/dev/null 2>&1; then
  ...
  echo "DKMS status:"
  dkms status 2>/dev/null | grep -i vbox || echo "  (no vbox dkms modules listed)"
else
  warn "dkms not installed. Guest modules usually still ship with the Parrot kernel."
fi
...
if command -v xclip >/dev/null 2>&1 && [ -n "${DISPLAY:-}" ]; then
  ...
  if [ "$CLIP_OUT" = "$TEST_TEXT" ]; then
    pass "Parrot internal X11 clipboard is working"
    echo
    echo "===================================================="
    echo " STUDENT TEST:"
